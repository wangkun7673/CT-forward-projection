...
for k = 1:size(ellipse,1)    
   ...
   % the default shape of the element is ellipse and you can change it to
   % rectangle
   idx = find((coordp(1,:)-x0).^2./asq + (coordp(2,:)-y0).^2./bsq + (coordp(3,:)-z0).^2./csq <= 1);
   % this is the definition of rectangle
   % idx = find((coordp(1,:)-x0).^2./asq <= 1 & (coordp(2,:)-y0).^2./bsq <= 1 & (coordp(3,:)-z0).^2./csq <= 1);
   % the shape can be also changed to cylinder
   % idx = find((coordp(1,:)-x0).^2./asq + (coordp(2,:)-y0).^2./bsq <= 1 & (coordp(3,:)-z0).^2./csq <= 1);
   p(idx) = p(idx) + A;
end
